<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      @change="handleFilter"
      >
      <el-option v-for="item in categoryList" :key="item.value" :label="item.label" :value="item.label"></el-option>
      </el-select>
      <el-button
      v-waves
      class="filter-item"
    ...
    <el-table-column label="封面图片" v-if="showCover" width="150"  align="center">
    ...
    <template slot-scope="{row}">
      <PreviewDialog title="电子信息" :data="row">
...
<script>
  import {listBook,getCategory,deleteBook
  } from '@/api/book'
...
export default {
  ...
  methods:{
    ...
    getCategoryList(){
      getCategory().then(respone=>{
        this.categoryList =respone.msg 
      })
    },
    getList(){
      this.listLoading = true;
      listBook(this.listQuery).then((respone)=>{
        const {
          data,
          total
        } = respone;
    ...
    handleDelete(row){
      this.$confirm('是否删除此电子书','提示',{
        confimButtonText:'确定',
        cancelButtonText:'取消'
      }).then(()=>{
          
          deleteBook(row.fileName).then(respone=>{
            this.$notify({
              title:'成功',
              message:respone.msg || '删除成功',
              type:'success',
              duration:2000
            })
            this.handleFilter()
